...
if ( ! class_exists( 'KPDNS_Cloudflare_Credentials' ) ) {

    class KPDNS_Cloudflare_Credentials extends KPDNS_Credentials {
    	...
    	public function __construct() {
    		...
    		$provider_id = KPDNS_Provider_Factory::CLOUDFLARE;
        ...
        public function render_fields() {
	        ...
	        <table class="form-table">
		        <tbody>
                    ...
                    <tr class="kpdns-<?php esc_attr_e( $this->provider_id ) ?>-field">
                        <th>
                            <label for="kpdns-settings-credentials-<?php esc_attr_e( $this->provider_id ); ?>-email"><?php _e( 'Email', 'keypress-dns' ); ?></label>
                        </th>
                        <td >
                            <input type="text" name="kpdns-settings[credentials][<?php esc_attr_e( $this->provider_id ); ?>][email]" id="kpdns-settings-credentials-<?php esc_attr_e( $this->provider_id ) ?>-email" class="regular-text ltr" value="<?php echo isset( $this->email ) ? esc_attr( $this->get_decrypted_field_value( $this->email ) ) : ''; ?>" />
                        </td>
                    </tr>
		        </tbody>
	        </table>
        ...
        public function save( $settings, $key ) {
    	    // Validate
            if ( ! isset( $settings ) || ! isset( $settings['api_key'] )  || ! isset( $settings['email'] ) ) {
                return __( 'Invalid credentials settings.', 'keypress-dns' );
            }

            $this->api_key = sanitize_text_field( wp_unslash( trim( $settings['api_key'] ) ) );
            $this->email = sanitize_text_field( wp_unslash( trim( $settings['email'] ) ) );
